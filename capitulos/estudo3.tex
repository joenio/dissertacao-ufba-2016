...
%Segundo o {\it ``Staged Model for Software Evolução''} \cite{rajlich2000staged},
...
%{\it``Evolução''}, {\it ``Manutenção''}, {\it ``Descontinuado''}, até o encerramento de seu ciclo
%no estágio de {\it ``Encerrado''}, quando as empresas detentoras do
...
%variar. No modelo {\it ``Staged Model for Software Evolução''} adaptado a
%projetos software livre \cite{capiluppi2007adapting}, por exemplo, o ciclo de
%vida do software não se encerra na fase de {\it ``Encerrado''}, uma vez que estes projetos
...
\citeonline{capiluppi2007adapting}; uma vez que não há entre eles projetos com
...
%    \texttt{s58}
%  \end{minipage}
%  \caption{Variação no número de módulos dos projetos em fase de {\it Manutenção}.}
