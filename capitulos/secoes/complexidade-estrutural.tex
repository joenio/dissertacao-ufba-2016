...
%Manutenibilidade é uma característica de qualidade externa que indica o quão
...
através de aprendizado ou evolução \cite{mitchell2009complexity}. As seguintes
...
objetivos e funções previamente definidos \cite{simon1996sciences}. Neste sentido,
...
manutenção \cite{terceiro2012understanding}. Ela está relacionada a como os módulos de um
...
\citeonline{darcy2005structural} como indicador da complexidade dos sistemas de software
em relação à sua estrutura interna e ao relacionamento entre os seus
componentes.

\citeonline{darcy2005structural} definem complexidade estrutural como uma combinação de
...
Uma formalização da métrica proposta por \citeonline{darcy2005structural} pode ser
...
\begin{itemize}
  ...
  \item {\bf CBO} {\it Coupling Between Objects (Acoplamento entre objetos)}:
    mede o acoplamento entre objetos do software \cite{chidamber1994metrics}
...
\cite{lincke2008comparing}. A definição original, por exemplo, inclui explicitamente
acoplamento via herança \cite{harrison1998evaluation}, no entando não deixa claro como
deve ser tratado métodos herdados \cite{briand1999using}. A definição original
...
a acessam \cite{sant2008modularity}.
...
testes, pois mais partes do sistema dependem delas.
  ...
  \item {\bf LCOM4} {\it Lack of Cohesion in Methods (Ausência de coesão em
    métodos)}: mede o grau de falta de coesão em métodos \cite{hitz1995measuring}.
...
seguir \cite{silva2012concernbased}.
...
A métrica de complexidade estrutural (SC) proposta por
\citeonline{darcy2005structural}, mede portanto, a complexidade do software em
termos dos valores das métricas de acoplamento (CBO) e coesão (LCOM4) e tem
sido utilizada como um importante fator de representação sobre o grau de
esforço necessário para se realizar atividades de manutenção em projetos de
software, ou seja, oferece fortes indícios sobre a manutenibilidade de um
software.
