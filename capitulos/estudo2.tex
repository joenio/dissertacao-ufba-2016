...
\texttt{documents/references.bib}, este arquivo contém os metadados de todos os
...
de menção inclui implicitamente os tipos de menor peso, este peso é pensado em
relação a contribuição da menção ao ecossistema do projeto de software, cada
...
A Listagem \ref{software-yml-references} apresenta um exemplo do trecho
do YML para armazenar a extração dessas informações.
...
\begin{figure}[h]
  ...
  references:
    p12:
      ...
      is_software_mentioned: no
    p34:
      review: cita o software como trabalho relacionado
      is_software_mentioned: yes
      mention_type: cite
    p40:
      review: cita o software como trabalho relacionado
      is_software_mentioned: yes
      mention_type: cite
  \end{verbatim}
  \caption{Trecho do arquivo software.yml (seção para armazenar as referências e menções).}
  \label{software-yml-references}
\end{figure}
...
\texttt{bin/ids} e \texttt{bin/render-template} indicados
